...
function ResultContent() {
    ...
    const hasKeypointsData = analysisResult?.skeleton_data?.keypoints &&
        Array.isArray(analysisResult.skeleton_data.keypoints) &&
        analysisResult.skeleton_data.keypoints.length > 0

    // Use skeleton video generated by backend (reliable approach)
    // The Next.js proxy rewrites /files/* to http://localhost:5000/files/*
    const skeletonVideoUrl = analysisResult?.skeleton_data?.skeleton_video_url || null

    // Fallback chain: skeleton video -> sample video
    const videoSrc = skeletonVideoUrl
        ? skeletonVideoUrl  // e.g., "/files/video_skeleton.mp4" - proxied through Next.js
    ...
    const keypointsData = (hasKeypointsData && !skeletonVideoUrl)
        ? analysisResult.skeleton_data.keypoints
        : undefined
    const keypointsFps = analysisResult?.skeleton_data?.fps || 30
