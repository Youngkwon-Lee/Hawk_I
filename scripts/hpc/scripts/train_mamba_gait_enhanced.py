"""
Mamba + Enhanced Features for Gait Task
Combining the best model (Mamba) with enhanced feature engineering for Gait
...
Usage:
    python scripts/train_mamba_gait_enhanced.py --epochs 200
"""
import os
import math
import numpy as np
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"Mamba + Enhanced Features (Gait) - 5-Fold CV")
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS - Mamba + Enhanced Features (Gait)")
    print('='*60)
...
def load_data(config):
    ...
    data_paths = [
        ...
        ("./data/gait_train_v4.pkl", "./data/gait_test_v4.pkl"),
        ("./data/gait_train_v2.pkl", "./data/gait_test_v2.pkl"),
    ]
...
def main():
    parser = argparse.ArgumentParser(description='Mamba + Enhanced Features for Gait Task')
    ...
    print(f"{'='*60}")
    print("MAMBA + ENHANCED FEATURES - GAIT TASK")
    print(f"{'='*60}")
    ...
    result_path = os.path.join(config.RESULT_DIR, f"mamba_gait_enhanced_{timestamp}.txt")

    with open(result_path, 'w') as f:
        f.write("Mamba + Enhanced Features - Gait Task\n")
