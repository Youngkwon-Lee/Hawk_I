"""
Mamba + Ordinal Regression (CORAL Loss)
UPDRS 0-4 scores are ordinal - use ordinal regression instead of MSE

CORAL (Consistent Rank Logits):
- Treats ordinal regression as multiple binary classification
- Predicts P(Y > k) for each threshold k
- Maintains natural ordering of classes

Usage:
    python scripts/train_finger_ordinal.py --epochs 200
"""
import os
import math
import numpy as np
...
class CoralLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        """
        logits: (batch_size, num_classes - 1) - one logit per threshold
        labels: (batch_size,) - ordinal labels 0 to num_classes-1
        """
        # Create ordinal encoding: for label k, create [1,1,...,1,0,0,...,0]
        # where first k elements are 1
...
def ordinal_to_label(logits):
    ...
    Count how many thresholds the prediction exceeds.
    """
    probs = torch.sigmoid(logits)
    # Sum probabilities that exceed 0.5 to get predicted class
...
# ============================================================
# Mamba Model with Ordinal Output
# ============================================================
class MambaOrdinal(nn.Module):
    """
    Mamba model with ordinal regression output.
    Outputs K-1 logits for K classes (UPDRS 0-4 → 4 logits)
    """
    def __init__(self, input_size, num_classes=5, hidden_size=256, num_layers=4, dropout=0.4):
        super().__init__()
        self.num_classes = num_classes
        ...
        # Ordinal output: num_classes - 1 logits
        self.classifier = nn.Sequential(
            nn.Linear(hidden_size, 128),
            nn.LayerNorm(128),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(128, num_classes - 1)  # 4 thresholds for 5 classes
        )
    ...
    def forward(self, x):
        ...
        return self.classifier(x)  # (batch, num_classes - 1)
...
def evaluate(model, X, y, device):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(X), 64):
            logits = model(torch.FloatTensor(X[i:i+64]).to(device))
        ...
        # Convert ordinal probabilities to expected value for correlation
        # E[Y] = sum(P(Y > k)) for k = 0 to K-1
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"Mamba + CORAL Ordinal Regression")
    print(f"Features: {X.shape[2]}, Classes: {config.NUM_CLASSES}")
    print('='*60)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y_binned)):
        ...
        model = MambaOrdinal(X.shape[2], config.NUM_CLASSES, config.HIDDEN_SIZE,
    ...
    # Correlation using expected values (continuous)
    pearson, _ = stats.pearsonr(y, all_expected)
    spearman, _ = stats.spearmanr(y, all_expected)

    # Also compute with discrete predictions
    pearson_discrete, _ = stats.pearsonr(y, all_preds)
    spearman_discrete, _ = stats.spearmanr(y, all_preds)

    print(f"\n{'='*60}")
    print(f"RESULTS - Mamba + CORAL Ordinal")
    print('='*60)
    print(f"MAE: {avg_mae:.3f}")
    print(f"Exact: {avg_exact:.1f}%")
    print(f"Within1: {avg_within1:.1f}%")
    print(f"Pearson (expected): {pearson:.3f}")
    print(f"Spearman (expected): {spearman:.3f}")
    print(f"Pearson (discrete): {pearson_discrete:.3f}")
    print(f"Spearman (discrete): {spearman_discrete:.3f}")

    return {
        'mae': avg_mae, 'exact': avg_exact, 'within1': avg_within1,
        'pearson': pearson, 'spearman': spearman,
        'pearson_discrete': pearson_discrete, 'spearman_discrete': spearman_discrete
    }
...
def main():
    parser = argparse.ArgumentParser(description='Mamba + CORAL Ordinal Training')
    ...
    print(f"{'='*60}")
    print("MAMBA + CORAL ORDINAL REGRESSION")
    print(f"{'='*60}")
    ...
    print("-" * 65)
    print(f"{'Mamba + Enhanced (MSE)':<35} {'0.444':>8} {'63.0%':>10} {'0.609':>10}")
    print(f"{'Mamba + CORAL Ordinal (this)':<35} {results['mae']:>8.3f} {results['exact']:>9.1f}% {results['pearson']:>10.3f}")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_path = os.path.join(config.RESULT_DIR, f"finger_ordinal_{timestamp}.txt")

    with open(result_path, 'w') as f:
        f.write("Mamba + CORAL Ordinal Regression\n")
        f.write(f"Date: {datetime.now()}\n")
        f.write(f"Device: {device}\n")
        f.write(f"Epochs: {config.EPOCHS}\n\n")

        f.write("Method:\n")
        f.write("  CORAL (Consistent Rank Logits) loss\n")
        f.write("  Treats ordinal regression as K-1 binary classifications\n")
        f.write("  Predicts P(Y > k) for each threshold k\n\n")
        ...
        f.write(f"  Pearson (expected): {results['pearson']:.3f}\n")
        f.write(f"  Spearman (expected): {results['spearman']:.3f}\n")
        f.write(f"  Pearson (discrete): {results['pearson_discrete']:.3f}\n")
        f.write(f"  Spearman (discrete): {results['spearman_discrete']:.3f}\n")
