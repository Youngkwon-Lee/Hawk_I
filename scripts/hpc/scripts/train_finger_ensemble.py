"""
Finger Tapping Ensemble: Mamba + Enhanced + Clinical
Combines best features from both approaches

Usage:
    python train_finger_ensemble.py --epochs 200
"""
import os
import math
import numpy as np
...
# ============================================================
# Feature Engineering (Enhanced + Clinical)
# ============================================================
...
class ClinicalFeatureEngineer:
    ...
    def add_clinical_features(X):
        ...
        for i in range(N):
            ...
            clinical_feat = np.array([sparc, amp_decline, freq_var, hesitation_frac])
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"Finger Ensemble (Enhanced + Clinical) - 5-Fold CV")
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS - Finger Ensemble")
    print('='*60)
...
def main():
    ...
    print(f"{'='*60}")
    print("FINGER ENSEMBLE (Enhanced + Clinical)")
    print(f"{'='*60}")
    ...
    print(f"{'Mamba + Clinical V1':<30} {'0.454':>8} {'63.7%':>10} {'0.578':>10}")
    print(f"{'Finger Ensemble':<30} {results['mae']:>8.3f} {results['exact']:>9.1f}% {results['pearson']:>10.3f}")

    # Save results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_path = os.path.join(config.RESULT_DIR, f"finger_ensemble_{timestamp}.txt")

    with open(result_path, 'w') as f:
        f.write("Finger Ensemble (Enhanced + Clinical)\n")
        f.write(f"Date: {datetime.now()}\n")
        f.write(f"Device: {device}\n")
        f.write(f"Epochs: {config.EPOCHS}\n\n")
        f.write(f"Features:\n")
        f.write(f"  Enhanced: velocity, acceleration, moving_stats\n")
        f.write(f"  Clinical: SPARC, amplitude_decline, freq_var, hesitation\n")
