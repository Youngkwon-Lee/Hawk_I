"""
Finger Ensemble: CORAL Ordinal (MAE 0.370, Pearson 0.555) + Enhanced MSE (MAE 0.444, Pearson 0.609)
Test multiple ensemble weights to find optimal combination

Usage:
    python scripts/train_finger_ensemble_coral_enhanced.py --epochs 200
"""
import os
import numpy as np
import pickle
...
def main():
    ...
    print(f"{'='*60}")
    print("FINGER ENSEMBLE: CORAL + ENHANCED")
    print(f"{'='*60}")
    ...
    for train_path, test_path in [
        (os.path.join(Config.DATA_DIR, "finger_tapping_train.pkl"), os.path.join(Config.DATA_DIR, "finger_tapping_test.pkl")),
        ("./finger_tapping_train.pkl", "./finger_tapping_test.pkl"),
    ]:
    ...
    result_path = os.path.join(Config.RESULT_DIR, f"finger_tapping_ensemble_coral_enhanced_{timestamp}.txt")

    with open(result_path, 'w') as f:
        f.write("Finger Ensemble: CORAL + Enhanced\n")
        f.write("=" * 60 + "\n")
