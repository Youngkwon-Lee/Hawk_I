"""
Gait Mamba + Clinical Features V1
Clinical features for Gait analysis

Usage:
    python train_gait_clinical_v1.py --epochs 200
"""
import os
import math
import numpy as np
...
class ClinicalFeatureEngineer:
    """Gait-specific clinical features"""
    ...
    def calculate_gait_symmetry(velocity):
        """Gait symmetry - left/right balance"""
        if len(velocity.shape) > 1:
            vel_mag = np.linalg.norm(velocity, axis=1)
        else:
            vel_mag = velocity

        # Split into first and second half (approximation of left/right)
        mid = len(vel_mag) // 2
        left = vel_mag[:mid]
    ...
    def add_clinical_features(X):
        print("Adding gait clinical features...")
        ...
        for i in range(N):
            ...
            clinical_feat = np.array([sparc, symmetry, stride_var, freezing_frac])
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"Gait Mamba + Clinical V1 - 5-Fold CV")
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS - Gait Clinical V1")
    print('='*60)
...
def main():
    ...
    print(f"{'Gait + Clinical V1':<30} {results['mae']:>8.3f} {results['exact']:>9.1f}% {results['pearson']:>10.3f}")

    # Save
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_path = os.path.join(config.RESULT_DIR, f"gait_clinical_v1_{timestamp}.txt")

    with open(result_path, 'w') as f:
        f.write("Gait Mamba + Clinical V1\n")
        f.write(f"Date: {datetime.now()}\n")
        f.write(f"Device: {device}\n")
        f.write(f"Epochs: {config.EPOCHS}\n\n")
        f.write("Clinical Features:\n")
        f.write("  1. SPARC smoothness\n")
        f.write("  2. Gait symmetry\n")
        f.write("  3. Stride variability\n")
        f.write("  4. Freezing of gait fraction\n\n")
