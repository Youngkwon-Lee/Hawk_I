"""
Mamba + CORAL Ordinal Regression for Hand Movement Task
Apply the successful CORAL approach to Hand Movement

Based on successful results:
- Gait CORAL: Pearson 0.807, MAE 0.241, Exact 76.5%
- Finger CORAL: Pearson 0.555, MAE 0.370, Exact 64.8%

Usage:
    python scripts/train_hand_ordinal.py --epochs 200
"""
import os
import math
import numpy as np
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"Mamba + CORAL Ordinal Regression (Hand Movement)")
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS - Mamba + CORAL Ordinal (Hand Movement)")
    print('='*60)
...
def main():
    parser = argparse.ArgumentParser(description='Mamba + CORAL Ordinal (Hand Movement)')
    ...
    print(f"{'='*60}")
    print("MAMBA + CORAL ORDINAL REGRESSION (HAND MOVEMENT)")
    print(f"{'='*60}")
    ...
    print("-" * 65)
    print(f"{'ActionMamba (Mamba+GCN)':<35} {'0.481':>8} {'54.5%':>10} {'0.511':>10}")
    print(f"{'Mamba + CORAL Ordinal (this)':<35} {results['mae']:>8.3f} {results['exact']:>9.1f}% {results['pearson']:>10.3f}")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_path = os.path.join(config.RESULT_DIR, f"hand_ordinal_{timestamp}.txt")

    with open(result_path, 'w') as f:
        f.write("Mamba + CORAL Ordinal Regression (Hand Movement)\n")
        ...
        f.write("Comparison:\n")
        f.write(f"  ActionMamba: MAE 0.481, Exact 54.5%, Pearson 0.511\n")
        f.write(f"  This model: MAE {results['mae']:.3f}, Exact {results['exact']:.1f}%, Pearson {results['pearson']:.3f}\n")
