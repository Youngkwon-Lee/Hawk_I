"""
ActionMamba: Mamba + GCN Hybrid for Leg Agility Analysis
...
# ============================================================
def get_leg_graph(num_nodes=33):
    """
...
class ActionMamba(nn.Module):
    ...
    def __init__(self, num_joints, in_channels, num_classes=5,
        ...
        self.A = get_leg_graph(num_joints)
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"ActionMamba: Mamba + GCN Hybrid (Leg Agility)")
...
def main():
    parser = argparse.ArgumentParser(description='ActionMamba (Leg Agility)')
    ...
    print(f"{'='*60}")
    print("ACTIONMAMBA: MAMBA + GCN HYBRID (LEG AGILITY)")
    print(f"{'='*60}")
    ...
    result_file = os.path.join(config.RESULT_DIR, f"action_mamba_leg_{timestamp}.txt")

    with open(result_file, 'w') as f:
        f.write("="*60 + "\n")
        f.write("ActionMamba: Mamba + GCN Hybrid (Leg Agility)\n")
        f.write("="*60 + "\n\n")
    ...
    print("-" * 65)
    print(f"{'CORAL Ordinal (Mamba only)':<35} {'0.241':>8} {'76.5%':>10} {'0.807':>10}")
    print(f"{'ActionMamba (Mamba + GCN)':<35} {results['mae']:>8.3f} {results['exact']:>9.1f}% {results['pearson']:>10.3f}")
    improvement = ((results['pearson'] - 0.807) / 0.807) * 100
