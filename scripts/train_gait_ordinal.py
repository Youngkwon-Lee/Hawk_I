"""
Mamba + CORAL Ordinal Regression for Gait Task
Apply the successful ordinal approach from Finger Tapping to Gait

CORAL Loss achieved on Finger:
- MAE: 0.370 (16.7% improvement)
- Exact: 64.8% (Best!)

Usage:
    python scripts/train_gait_ordinal.py --epochs 200
"""
import os
import math
import numpy as np
...
# ============================================================
# Feature Engineering (Same as Gait Enhanced)
# ============================================================
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"Mamba + CORAL Ordinal Regression (Gait)")
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS - Mamba + CORAL Ordinal (Gait)")
    print('='*60)
...
def main():
    parser = argparse.ArgumentParser(description='Mamba + CORAL Ordinal (Gait)')
    ...
    print(f"{'='*60}")
    print("MAMBA + CORAL ORDINAL REGRESSION (GAIT)")
    print(f"{'='*60}")
    ...
    print("-" * 65)
    print(f"{'Mamba + Enhanced (MSE)':<35} {'0.335':>8} {'71.9%':>10} {'0.804':>10}")
    print(f"{'Mamba + CORAL Ordinal (this)':<35} {results['mae']:>8.3f} {results['exact']:>9.1f}% {results['pearson']:>10.3f}")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_path = os.path.join(config.RESULT_DIR, f"gait_ordinal_{timestamp}.txt")

    with open(result_path, 'w') as f:
        f.write("Mamba + CORAL Ordinal Regression (Gait)\n")
