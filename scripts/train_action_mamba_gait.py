"""
ActionMamba: Mamba + GCN Hybrid for Gait Analysis
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"ActionMamba: Mamba + GCN Hybrid (Gait)")
...
def main():
    parser = argparse.ArgumentParser(description='ActionMamba (Gait)')
    ...
    print(f"{'='*60}")
    print("ACTIONMAMBA: MAMBA + GCN HYBRID (GAIT)")
    print(f"{'='*60}")
    ...
    result_file = os.path.join(config.RESULT_DIR, f"action_mamba_gait_{timestamp}.txt")

    with open(result_file, 'w') as f:
        f.write("="*60 + "\n")
        f.write("ActionMamba: Mamba + GCN Hybrid (Gait)\n")
        f.write("="*60 + "\n\n")
