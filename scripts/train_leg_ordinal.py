"""
Mamba + CORAL Ordinal Regression for Leg Agility Task
Apply the successful CORAL approach to Leg Agility
...
Usage:
    python scripts/train_leg_ordinal.py --epochs 200
"""
import os
import math
import numpy as np
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"Mamba + CORAL Ordinal Regression (Leg Agility)")
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS - Mamba + CORAL Ordinal (Leg Agility)")
    print('='*60)
...
def main():
    parser = argparse.ArgumentParser(description='Mamba + CORAL Ordinal (Leg Agility)')
    ...
    print(f"{'='*60}")
    print("MAMBA + CORAL ORDINAL REGRESSION (LEG AGILITY)")
    print(f"{'='*60}")
    ...
    print("-" * 65)
    print(f"{'ActionMamba (Mamba+GCN)':<35} {'0.486':>8} {'55.7%':>10} {'0.195':>10}")
    print(f"{'Mamba + CORAL Ordinal (this)':<35} {results['mae']:>8.3f} {results['exact']:>9.1f}% {results['pearson']:>10.3f}")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_path = os.path.join(config.RESULT_DIR, f"leg_ordinal_{timestamp}.txt")

    with open(result_path, 'w') as f:
        f.write("Mamba + CORAL Ordinal Regression (Leg Agility)\n")
        ...
        f.write("Comparison:\n")
        f.write(f"  ActionMamba: MAE 0.486, Exact 55.7%, Pearson 0.195\n")
