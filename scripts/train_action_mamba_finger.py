"""
ActionMamba: Mamba + GCN Hybrid for Finger Tapping Analysis
...
def kfold_cv(X, y, config, device):
    print(f"\n{'='*60}")
    print(f"ActionMamba: Mamba + GCN Hybrid (Finger Tapping)")
...
def main():
    parser = argparse.ArgumentParser(description='ActionMamba (Finger Tapping)')
    ...
    result_file = os.path.join(config.RESULT_DIR, f"action_mamba_finger_{timestamp}.txt")

    with open(result_file, 'w') as f:
        f.write("="*60 + "\n")
        f.write("ActionMamba: Mamba + GCN Hybrid (Finger Tapping)\n")
        f.write("="*60 + "\n\n")
