...
class VisionAgent(BaseAgent):
    ...
    def process(self, ctx: AnalysisContext) -> AnalysisContext:
        try:
            ...
            if ctx.manual_test_type:
                ctx.task_type = ctx.manual_test_type
                manual_body_part = "hand" if ctx.task_type in ["finger_tapping", "hand_movement", "pronation_supination"] else "fullbody"
                ctx.vision_meta.update({
                    "roi": (0, 0, frame_width, frame_height),
                    "body_part": manual_body_part,
                    "motion_pattern": "manual_override",
                    "motion_area_ratio": 1.0,
                    "confidence": 1.0,
                    "reasoning": f"Manual task override used: {ctx.manual_test_type}",
                    "heatmap_path": None,
                })
                ctx.log("vision", f"Manual task override applied: {ctx.task_type}", meta=dict(ctx.vision_meta))
            else:
                # 2. ROI Detection
                t0 = time.time()
