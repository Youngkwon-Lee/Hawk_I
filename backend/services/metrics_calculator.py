...
def safe_ratio(numerator: float, denominator: float, default: float = 0.0) -> float:
    """Alias for safe_divide to keep feature code readable."""
...
class FingerTappingMetrics:
    ...
    # === SEQUENCE-AUGMENTED FEATURES (finger severe-case cues) ===
    finger_distance_mean: float
    finger_distance_std: float
    norm_distance_mean: float
    norm_distance_std: float
    dist_velocity_mean: float
    dist_velocity_std: float
    dist_accel_mean: float
    dist_accel_std: float
    index_tip_y_rel_mean: float
    index_tip_y_rel_std: float
    index_tip_y_vel_mean: float
    index_tip_y_vel_std: float
    index_tip_y_acc_mean: float
    index_tip_y_acc_std: float
    thumb_palm_norm_mean: float
    thumb_palm_norm_std: float
    index_palm_norm_mean: float
    index_palm_norm_std: float
    peak_count: float
    begin_end_peak_ratio: float
    longest_pause: float
    pause_ratio: float
    below_threshold_cycle_ratio: float
    effective_tap_count: float
    effective_tap_ratio: float
    effective_tap_rate: float
    incomplete_tap_ratio: float
    onset_delay_ratio: float
    longest_low_amplitude_segment: float
    longest_low_amplitude_segment_ratio: float
    post_onset_pause_ratio: float
    low_amplitude_peak_ratio: float
    effective_peak_mean: float
    effective_begin_end_ratio: float
...
class MetricsCalculator:
    ...
    def calculate_finger_tapping_metrics(self, landmark_frames: List[Dict]) -> FingerTappingMetrics:
        ...
        timestamps = []
        wrist_positions = []
        index_tip_y_rel_values = []
        thumb_palm_norm_values = []
        index_palm_norm_values = []
        ...
        for frame in landmark_frames:
            ...
            if all(k in kp_dict for k in required):
                ...
                middle_mcp = np.array([kp_dict[9]['x'], kp_dict[9]['y'], kp_dict[9]['z']])
                ring_mcp = np.array([kp_dict[13]['x'], kp_dict[13]['y'], kp_dict[13]['z']])
                pinky_mcp = np.array([kp_dict[17]['x'], kp_dict[17]['y'], kp_dict[17]['z']])
                palm_center = np.mean(
                    np.stack([wrist, index_mcp, middle_mcp, ring_mcp, pinky_mcp], axis=0),
                    axis=0,
                )
                ...
                index_finger_lengths.append(finger_length)

                wrist_positions.append(wrist)
                index_tip_y_rel_values.append(float(index_tip[1] - palm_center[1]))
                scale = max(float(finger_length), EPSILON)
                thumb_palm_norm_values.append(float(np.linalg.norm(thumb_tip - palm_center) / scale))
                index_palm_norm_values.append(float(np.linalg.norm(index_tip - palm_center) / scale))
        ...
        index_tip_y_rel = np.array(index_tip_y_rel_values)
        thumb_palm_norm = np.array(thumb_palm_norm_values)
        index_palm_norm = np.array(index_palm_norm_values)
        ...
        index_tip_y_vel = np.diff(index_tip_y_rel, prepend=index_tip_y_rel[0]) if len(index_tip_y_rel) > 0 else np.array([])
        index_tip_y_acc = np.diff(index_tip_y_vel, prepend=index_tip_y_vel[0]) if len(index_tip_y_vel) > 0 else np.array([])
        ...
        # ===== SEQUENCE-AUGMENTED FEATURES =====
        finger_distance_mean = float(np.mean(distances))
        finger_distance_std = float(np.std(distances))
        norm_distance_mean = float(np.mean(distances_normalized))
        norm_distance_std = float(np.std(distances_normalized))
        dist_velocity_mean = float(np.mean(np.abs(velocity))) if len(velocity) > 0 else 0.0
        dist_velocity_std = float(np.std(velocity)) if len(velocity) > 0 else 0.0
        dist_accel_mean = float(np.mean(np.abs(acceleration))) if len(acceleration) > 0 else 0.0
        dist_accel_std = float(np.std(acceleration)) if len(acceleration) > 0 else 0.0

        index_tip_y_rel_mean = float(np.mean(index_tip_y_rel)) if len(index_tip_y_rel) > 0 else 0.0
        index_tip_y_rel_std = float(np.std(index_tip_y_rel)) if len(index_tip_y_rel) > 0 else 0.0
        index_tip_y_vel_mean = float(np.mean(np.abs(index_tip_y_vel))) if len(index_tip_y_vel) > 0 else 0.0
        index_tip_y_vel_std = float(np.std(index_tip_y_vel)) if len(index_tip_y_vel) > 0 else 0.0
        index_tip_y_acc_mean = float(np.mean(np.abs(index_tip_y_acc))) if len(index_tip_y_acc) > 0 else 0.0
        index_tip_y_acc_std = float(np.std(index_tip_y_acc)) if len(index_tip_y_acc) > 0 else 0.0

        thumb_palm_norm_mean = float(np.mean(thumb_palm_norm)) if len(thumb_palm_norm) > 0 else 0.0
        thumb_palm_norm_std = float(np.std(thumb_palm_norm)) if len(thumb_palm_norm) > 0 else 0.0
        index_palm_norm_mean = float(np.mean(index_palm_norm)) if len(index_palm_norm) > 0 else 0.0
        index_palm_norm_std = float(np.std(index_palm_norm)) if len(index_palm_norm) > 0 else 0.0

        peak_count = float(len(peaks))
        peak_vals = distances_normalized[peaks] if len(peaks) > 0 else np.array([])
        half_peak = max(1, len(peak_vals) // 2) if len(peak_vals) > 0 else 1
        if len(peak_vals) > 0:
            begin_peak_mean = float(np.mean(peak_vals[:half_peak]))
            end_peak_mean = float(np.mean(peak_vals[-half_peak:]))
            begin_end_peak_ratio = safe_divide(end_peak_mean, begin_peak_mean, 0.0)
        else:
            begin_end_peak_ratio = 0.0

        if len(peaks) > 1:
            peak_intervals = np.diff(peaks)
            longest_pause = float(np.max(peak_intervals))
            pause_ratio = safe_divide(longest_pause, float(np.mean(peak_intervals)), 0.0)
        else:
            longest_pause = float(len(distances_normalized))
            pause_ratio = 0.0

        threshold = 0.4 * float(np.percentile(distances_normalized, 90)) if len(distances_normalized) > 0 else 0.0
        below_threshold_cycle_ratio = (
            float(np.mean(distances_normalized < threshold)) if threshold > 0 else 0.0
        )

        high_ref = float(np.percentile(distances_normalized, 90)) if len(distances_normalized) > 0 else 0.0
        effective_peak_threshold = 0.6 * high_ref if high_ref > 0 else 0.0
        low_threshold = 0.35 * high_ref if high_ref > 0 else 0.0
        effective_peak_mask = peak_vals >= effective_peak_threshold if len(peak_vals) > 0 else np.array([], dtype=bool)
        effective_peaks = peaks[effective_peak_mask] if len(peaks) > 0 else np.array([], dtype=int)
        effective_peak_vals = peak_vals[effective_peak_mask] if len(peak_vals) > 0 else np.array([])

        effective_tap_count = float(len(effective_peaks))
        peak_count_value = float(len(peaks))
        effective_tap_ratio = safe_divide(effective_tap_count, peak_count_value, 0.0)
        incomplete_tap_ratio = 1.0 - effective_tap_ratio if peak_count_value > 0 else 0.0
        effective_tap_rate = safe_divide(effective_tap_count, float(len(distances_normalized)), 0.0)
        onset_delay_ratio = safe_divide(float(effective_peaks[0]), float(len(distances_normalized)), 1.0) if len(effective_peaks) > 0 else 1.0

        low_amplitude_mask = distances_normalized < low_threshold if low_threshold > 0 else np.zeros(len(distances_normalized), dtype=bool)
        longest_low_amplitude_segment = float(longest_true_run(low_amplitude_mask))
        longest_low_amplitude_segment_ratio = safe_ratio(longest_low_amplitude_segment, float(len(distances_normalized)))

        if len(effective_peaks) > 1:
            effective_intervals = np.diff(effective_peaks)
            post_onset_pause_ratio = safe_divide(float(np.max(effective_intervals)), float(np.mean(effective_intervals)), 0.0)
        else:
            post_onset_pause_ratio = 0.0

        low_amplitude_peak_ratio = (
            float(np.mean(peak_vals < effective_peak_threshold))
            if len(peak_vals) > 0 and effective_peak_threshold > 0
            else 0.0
        )
        effective_peak_mean = float(np.mean(effective_peak_vals)) if len(effective_peak_vals) > 0 else 0.0
        if len(effective_peak_vals) > 0:
            half_eff = max(1, len(effective_peak_vals) // 2)
            effective_begin_end_ratio = safe_divide(
                float(np.mean(effective_peak_vals[-half_eff:])),
                float(np.mean(effective_peak_vals[:half_eff])),
                0.0,
            )
        else:
            effective_begin_end_ratio = 0.0
        ...
        return FingerTappingMetrics(
            ...
            finger_distance_mean=finger_distance_mean,
            finger_distance_std=finger_distance_std,
            norm_distance_mean=norm_distance_mean,
            norm_distance_std=norm_distance_std,
            dist_velocity_mean=dist_velocity_mean,
            dist_velocity_std=dist_velocity_std,
            dist_accel_mean=dist_accel_mean,
            dist_accel_std=dist_accel_std,
            index_tip_y_rel_mean=index_tip_y_rel_mean,
            index_tip_y_rel_std=index_tip_y_rel_std,
            index_tip_y_vel_mean=index_tip_y_vel_mean,
            index_tip_y_vel_std=index_tip_y_vel_std,
            index_tip_y_acc_mean=index_tip_y_acc_mean,
            index_tip_y_acc_std=index_tip_y_acc_std,
            thumb_palm_norm_mean=thumb_palm_norm_mean,
            thumb_palm_norm_std=thumb_palm_norm_std,
            index_palm_norm_mean=index_palm_norm_mean,
            index_palm_norm_std=index_palm_norm_std,
            peak_count=peak_count,
            begin_end_peak_ratio=float(begin_end_peak_ratio),
            longest_pause=longest_pause,
            pause_ratio=float(pause_ratio),
            below_threshold_cycle_ratio=below_threshold_cycle_ratio,
            effective_tap_count=effective_tap_count,
            effective_tap_ratio=float(effective_tap_ratio),
            effective_tap_rate=float(effective_tap_rate),
            incomplete_tap_ratio=float(incomplete_tap_ratio),
            onset_delay_ratio=float(onset_delay_ratio),
            longest_low_amplitude_segment=longest_low_amplitude_segment,
            longest_low_amplitude_segment_ratio=float(longest_low_amplitude_segment_ratio),
            post_onset_pause_ratio=float(post_onset_pause_ratio),
            low_amplitude_peak_ratio=float(low_amplitude_peak_ratio),
            effective_peak_mean=float(effective_peak_mean),
            effective_begin_end_ratio=float(effective_begin_end_ratio),
        )
