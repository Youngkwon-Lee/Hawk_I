...
class MLScorer:
    ...
    def load_models(self, model_dir: str = ML_MODEL_DIR) -> bool:
        ...
        ft_feature_best = os.path.join(model_dir, "finger_feature_baseline_best_v0_3.joblib")
        if os.path.exists(ft_feature_best):
            try:
                self._models["finger_tapping_feature_baseline"] = joblib.load(ft_feature_best)
            except Exception:
                pass
    ...
    def _extract_metrics_dict(self, metrics: Any, task: str) -> dict:
        if hasattr(metrics, "__dataclass_fields__"):
            metrics_dict = asdict(metrics)
        elif isinstance(metrics, dict):
    ...
    def predict_finger_tapping(self, metrics, model_type: str = "rf") -> Optional[MLPrediction]:
        if not self._loaded:
            self.load_models()
        if model_type == "feature_baseline":
            return self.predict_finger_tapping_feature_baseline(metrics)
    ...
    def predict_finger_tapping_feature_baseline(self, metrics) -> Optional[MLPrediction]:
        if not self._loaded:
            self.load_models()

        package = self._models.get("finger_tapping_feature_baseline")
        if package is None:
            return None
        ...
        metrics_dict = self._extract_metrics_dict(metrics, "finger_tapping")
        try:
            import pandas as pd
            X = pd.DataFrame([{col: float(metrics_dict.get(col, 0.0) or 0.0) for col in feature_columns}])
        except Exception:
            X = np.array([[float(metrics_dict.get(col, 0.0) or 0.0) for col in feature_columns]], dtype=float)
            X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
        raw_pred = float(model.predict(X)[0])
        score = int(np.clip(np.round(raw_pred), 0, 4))
        confidence = max(0.0, min(1.0, 1.0 - abs(raw_pred - score)))
        return MLPrediction(
            score=score,
            confidence=round(confidence, 3),
            raw_prediction=round(raw_pred, 3),
            model_type="feature_baseline",
        )
